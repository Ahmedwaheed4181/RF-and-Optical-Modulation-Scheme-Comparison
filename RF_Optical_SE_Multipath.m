...
for snrIdx = 1:length(SNR_dB)
    ...
    if estimateChannel
        ...
        H_est = fft(rxPilot) ./ pilotSymbols;
    else
    ...
    se_DCOOFDM(snrIdx) = mean(capacity_per_sc) * (N/2-1)/N * (N/(N+cp_len));

    % ACO-OFDM
    tx_power = 1;
    effective_snr = snr_lin;
    capacity_per_sc = log2(1 + (abs(H_est_ACO).^2 * effective_snr));
    se_ACOOFDM(snrIdx) = mean(capacity_per_sc) * (N/4)/N * (N/(N+cp_len));
    ...
    effective_snr = snr_lin / (1 + dc_power/tx_power);
    doppler_factor = 0.05;
    H_est_OTFS = H_est_DCO .* (1 + doppler_factor*randn(size(H_est_DCO)));
    capacity_per_sc = log2(1 + (abs(H_est_OTFS).^2 * effective_snr));
    se_DCOTFS(snrIdx) = mean(capacity_per_sc) * (N/2-1)/N * (N/(N+cp_len));

    % ACO-OTFS
    tx_power = 1;
    effective_snr = snr_lin;
    H_est_OTFS = H_est_ACO .* (1 + doppler_factor*randn(size(H_est_ACO)));
    capacity_per_sc = log2(1 + (abs(H_est_OTFS).^2 * effective_snr));
    se_ACOTFS(snrIdx) = mean(capacity_per_sc) * (N/4)/N * (N/(N+cp_len));
    ...
    se_DCOCDM(snrIdx) = mean(capacity_per_sc) * (N/2-1)/N * (N/(N+cp_len));
    ...
    se_ACOCDM(snrIdx) = mean(capacity_per_sc) * (N/4)/N * (N/(N+cp_len));
    ...
    se_DCOOFDMIM(snrIdx) = mean(capacity_per_group) * (bits_per_group/group_size) * (N/(N+cp_len));
    ...
    capacity_per_group = log2(1 + effective_snr_IM(1:active_groups));
    se_ACOOFDMIM(snrIdx) = mean(capacity_per_group) * (bits_per_group/group_size) * (N/(N+cp_len));
    ...
    dc_power = dcBias^2;
    effective_snr = snr_lin / (1 + dc_power/tx_power);
    H_est_scma = abs(fft(channelTaps, scma_resources_optical)).^2;
    capacity_per_resource = log2(1 + H_est_scma * effective_snr);
    se_DCOSCMA(snrIdx) = scma_users_optical * mean(capacity_per_resource) * (scma_resources_optical/(scma_resources_optical+cp_len));
    ...
    H_est_scma = abs(fft(channelTaps, scma_resources_optical)).^2;
    capacity_per_resource = log2(1 + H_est_scma * effective_snr);
    se_ACOSCMA(snrIdx) = scma_users_optical * mean(capacity_per_resource) * (scma_resources_optical/(scma_resources_optical+cp_len));

    %% =================== RF Variants ===================
    % OFDM
    capacity_OFDM = mean(log2(1 + (abs(H_est).^2 * snr_lin)));
    se_OFDM(snrIdx) = capacity_OFDM * (N/(N+cp_len));

    % OTFS
    doppler_factor = 0.1;
    effective_snr_otfs = (abs(H_est).^2)*snr_lin./(1 + doppler_factor*rand(N,1));
    se_OTFS(snrIdx) = mean(log2(1 + effective_snr_otfs)) * (N/(N+cp_len));
    ...
    se_OCDM(snrIdx) = mean(log2(1 + effective_snr_ocdm)) * (N/(N+cp_len));
    ...
    se_OFDMIM(snrIdx) = mean(capacity_per_group) * (bits_per_group_rf/(group_size_rf+cp_len));

    % SCMA
    H_est_scma = abs(fft(channelTaps, scma_resources_rf)).^2;
    effective_snr_scma = H_est_scma * snr_lin;
    se_SCMA(snrIdx) = scma_users_rf * log2(1 + mean(effective_snr_scma)) * (scma_resources_rf/(scma_resources_rf+cp_len));
end

figure;
plot(SNR_dB, se_DCOOFDM, '-o', 'Color', [0.4660, 0.6740, 0.1880], 'LineWidth', 1.5, 'DisplayName', 'DCO-OFDM'); hold on;
plot(SNR_dB, se_DCOTFS, '-x', 'Color', [0.4660, 0.6740, 0.1880], 'LineWidth', 1.5, 'DisplayName', 'DCO-OTFS');
plot(SNR_dB, se_DCOCDM, '-+', 'Color', [0.4660, 0.6740, 0.1880], 'LineWidth', 1.5, 'DisplayName', 'DCO-OCDM');
plot(SNR_dB, se_DCOOFDMIM, '-s', 'Color', [0.4660, 0.6740, 0.1880], 'LineWidth', 1.5, 'DisplayName', 'DCO-OFDM-IM');
plot(SNR_dB, se_DCOSCMA, '-d', 'Color', [0.4660, 0.6740, 0.1880], 'LineWidth', 1.5, 'DisplayName', 'DCO-SCMA');

% ACO variants (dashed lines - slightly darker)
plot(SNR_dB, se_ACOOFDM, '--o', 'Color', [0.1960, 0.4740, 0.0880], 'LineWidth', 1.5, 'DisplayName', 'ACO-OFDM');
plot(SNR_dB, se_ACOTFS, '--x', 'Color', [0.1960, 0.4740, 0.0880], 'LineWidth', 1.5, 'DisplayName', 'ACO-OTFS');
plot(SNR_dB, se_ACOCDM, '--+', 'Color', [0.1960, 0.4740, 0.0880], 'LineWidth', 1.5, 'DisplayName', 'ACO-OCDM');
plot(SNR_dB, se_ACOOFDMIM, '--s', 'Color', [0.1960, 0.4740, 0.0880], 'LineWidth', 1.5, 'DisplayName', 'ACO-OFDM-IM');
plot(SNR_dB, se_ACOSCMA, '--d', 'Color', [0.1960, 0.4740, 0.0880], 'LineWidth', 1.5, 'DisplayName', 'ACO-SCMA');

xlabel('SNR (dB)'); 
ylabel('Spectral Efficiency (bits/s/Hz)');
title('Spectral Efficiency of Optical Variants');
...
% RF Variants
figure;
plot(SNR_dB, se_OFDM, '-o', 'Color', [0, 0.4470, 0.7410], 'LineWidth', 1.5, 'DisplayName', 'OFDM'); hold on;
plot(SNR_dB, se_OTFS, '-x', 'Color', [0, 0.4470, 0.7410], 'LineWidth', 1.5, 'DisplayName', 'OTFS');
plot(SNR_dB, se_OCDM, '-+', 'Color', [0, 0.4470, 0.7410], 'LineWidth', 1.5, 'DisplayName', 'OCDM');
plot(SNR_dB, se_OFDMIM, '-s', 'Color', [0, 0.4470, 0.7410], 'LineWidth', 1.5, 'DisplayName', 'OFDM-IM');
plot(SNR_dB, se_SCMA, '-d', 'Color', [0, 0.4470, 0.7410], 'LineWidth', 1.5, 'DisplayName', 'SCMA');
plot(SNR_dB, se_ACOOFDM, '--o', 'Color', [0.1960, 0.4740, 0.0880], 'LineWidth', 1.5, 'DisplayName', 'ACO-OFDM');
plot(SNR_dB, se_ACOTFS, '--x', 'Color', [0.1960, 0.4740, 0.0880], 'LineWidth', 1.5, 'DisplayName', 'ACO-OTFS');
plot(SNR_dB, se_DCOCDM, '-+', 'Color', [0.4660, 0.6740, 0.1880], 'LineWidth', 1.5, 'DisplayName', 'DCO-OCDM');
plot(SNR_dB, se_ACOOFDMIM, '--s', 'Color', [0.1960, 0.4740, 0.0880], 'LineWidth', 1.5, 'DisplayName', 'ACO-OFDM-IM');
...
title('RF and best from Optical');
legend('show');
grid on;
set(gca, 'FontSize', 12);
