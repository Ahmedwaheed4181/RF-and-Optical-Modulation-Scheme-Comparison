...
% No multipath channel - only AWGN
cp_len = 0;             % No need for cyclic prefix in AWGN only
...
for snrIdx = 1:length(SNR_dB)
    ...
    % Add CP (though not needed for AWGN)
    tx_OFDM_CP = [tx_OFDM(end-cp_len+1:end, :); tx_OFDM];
    
    % AWGN only
    rx_OFDM_CP = awgn(tx_OFDM_CP, snr, 'measured');
    ...
    % Add CP and transmit through AWGN
    tx_DCOOFDM_CP = [tx_DCOOFDM(end-cp_len+1:end, :); tx_DCOOFDM];
    rx_DCOOFDM_CP = awgn(tx_DCOOFDM_CP, snr, 'measured');
   ...
   %% =================== ACO-OFDM ===================
    ...
    rx_ACOOFDM_CP = awgn(tx_ACOOFDM_CP, snr, 'measured');
    ...
    rx_OTFS_CP = awgn(tx_OTFS_CP, snr, 'measured');
...
rx_DCOTFS_CP = awgn(tx_DCOTFS_CP, snr, 'measured');
...
rx_ACOTFS_CP = awgn(tx_ACOTFS_CP, snr, 'measured');
...
ber_ACOTFS(snrIdx) = mean(bits_ACOTFS ~= rxBits_ACOTFS(1:length(bits_ACOTFS)));
    ...
    rx_OCDM_CP = awgn(tx_OCDM_CP, snr, 'measured');
...
rx_DCOCDM_CP = awgn(tx_DCOCDM_CP, snr, 'measured');
...
rx_ACOCDM_CP = awgn(tx_ACOCDM_CP, snr, 'measured');
...
ber_ACOCDM(snrIdx) = mean(bits_ACOCDM ~= rxBits_ACOCDM(1:length(bits_ACOCDM)));
    ...
    % AWGN only
    rx_OFDMIM_CP = awgn(tx_OFDMIM_CP, snr, 'measured');
...
rx_DCOOFDMIM_CP = awgn(tx_DCOOFDMIM_CP, snr, 'measured');
...
rx_ACOOFDMIM_CP = awgn(tx_ACOOFDMIM_CP, snr, 'measured');
...
ber_ACOOFDMIM(snrIdx) = mean(bits_ACOOFDMIM ~= rx_bits_ACOIM(1:length(bits_ACOOFDMIM)));
    ...
    cp_len_scma = 0;  % No CP needed for AWGN
    ...
    % AWGN only
    rx_SCMA = awgn(tx_SCMA, snr, 'measured');
    ...
    for sym = 1:numSymbols
        ...
        for b1 = 0:M-1
            for b2 = 0:M-1
                ...
                % Calculate distance (no channel effect)
                dist = sum(abs(rx_SCMA(:, sym) - cw).^2);
...
% SCMA encoding with Hermitian symmetry
   %% DCO-SCMA (DC-Biased Optical SCMA)
    ...
    rx_DCOSCMA_CP = awgn(tx_DCOSCMA_CP, snr, 'measured');
    ...
    for sym = 1:numSymbols
        ...
        for b1 = 0:M-1
            for b2 = 0:M-1
                ...
                dist = sum(abs(rx_DCOSCMA(:, sym) - tx_sym).^2);
    ...
    rx_ACOSCMA_CP = awgn(tx_ACOSCMA_CP, snr, 'measured');
    ...
    for sym = 1:numSymbols
        ...
        for b1 = 0:M-1
            for b2 = 0:M-1
                ...
                % Compare with received signal
                dist = sum(abs(rx_ACOSCMA(:, sym) - tx_sym).^2);
